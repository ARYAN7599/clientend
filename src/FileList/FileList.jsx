...
const baseURL= "http://blockchaintimes.live/images/";

const FileList = ({ files, getFile }) => {
    const getFileHandler = (_name) => {
        axios.get(`http://blockchaintimes.live/images/${_name}`)
            .then((res) => getFile("http://blockchaintimes.live/images/"+_name))
            .catch((err) => console.error(err));
    }
    const [imageArray, setPost] = useState(null);
    const get = async () => {
        axios.get("http://blockchaintimes.live").then((res) => {
            setPost(res.data);
        });
    }
